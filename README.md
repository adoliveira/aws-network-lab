...
Para que a pipeline do GitHub Actions funcione corretamente, você precisa configurar os seguintes secrets no seu repositório:
...
```bash
# 1. Crie um usuário IAM com as permissões necessárias
# 2. Gere Access Keys para o usuário
# 3. Configure os secrets no GitHub
```

#### Opção 2: AWS CLI Local
```bash
# Se você já tem AWS CLI configurado:
aws configure list
```

### Permissões Mínimas Necessárias

O usuário/role AWS precisa das seguintes permissões para validação:
- `ec2:DescribeRegions`
- `ec2:DescribeAvailabilityZones`
- `ec2:DescribeVpcs` (para validação)
...
- **Subnets Públicas** (1 por AZ) com IP público **sob demanda** (segurança aprimorada)
